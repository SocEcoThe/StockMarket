...
由于原版项目缺乏一些运行库和环境兼容的问题，这边改写了一些内容，让插件能顺利运行<br>
版本依赖为1.18
## 更改项如下
1. 删除了缺失的xjcraft依赖库，增加了猜测的功能实现代码
2. 删除了在服务端有些兼容问题（环境为authlib，有类加载器和数据不写入的情况出现）的io.ebean，改为sql+hikari形式
3. 原先默认图标存在难以分辨功能（没有更改方块名称或数字提示缺失），改写了默认的商店图标，并增加了提示
